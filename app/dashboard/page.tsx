...
:root{
  ...
  --menu-h:65vh;--card-gap:14px;--sb-w:210px;--ca-pad:52px;
  --visible-cards:4;
...
// ─── Script du header (exécuté APRÈS le rendu du HTML) ──────────────────────
...
export default function DashboardPage() {
  ...
  // ── Header HTML injecté → on attend le prochain tick pour lancer le script ──
  useEffect(() => {
    if (headerReady) {
      // Logout handlers
      const logout = async () => {
      ...
      // Script header — injecté UNE SEULE FOIS après que le DOM est prêt
      const existing = document.getElementById('ms-header-script')
      if (!existing) {
      ...
      return () => {
        ids.forEach(id => document.getElementById(id)?.removeEventListener('click', logout))
      }
    }
  }, [headerReady])
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#fff', fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif', color: '#111' }}>
      ...
      {/* ── HTML du header — ref callback pour déclencher le script après rendu ── */}
      <div
        ref={(node) => { if (node && !headerReady) setHeaderReady(true) }}
      ...
      <style>{`
        ...
        /* Contenu — padding-top = hauteur header desktop */
        ...
        .ms-myms-grid {
          ...
          gap: 60px;
          /* align-items: start → WELCOME et description partent du même niveau */
          align-items: start;
      ...
      <div className="ms-tabs-bar">
        <div className="ms-tabs-inner">
          {([
            ...
            <button
              key={key}
              className={`ms-tab-btn${tab === key ? ' active' : ''}`}
              onClick={() => setTab(key)}
            >
              {label}
            </button>
          ...
          <div className="ms-myms-grid">
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', paddingTop: '4px' }}>
              <button
                ...
                style={{ background: '#111', color: '#fff', border: 'none', padding: '16px 32px', fontSize: '11px', letterSpacing: '2px', textTransform: 'uppercase', cursor: 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap' }}
              >
                MY EXPERIENCES
              </button>
            </div>
